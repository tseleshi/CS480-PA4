...
class Sketch(CanvasBase):
    ...
    #toggle lights
    specular_toggle = False  # True for on, False for off
    diffuse_toggle = False  # True for on, False for off
    ambient_toggle = False  # True for on, False for off
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        #implemented scene switching (1-4)
        if chr(keycode) in "1":
            self.switchScene(SceneOne(self.shaderProg))
            self.update()
        if chr(keycode) in "2":
        ...
        # Toggle on/off specular, diffuse, ambient (only works on 1 light)
        if chr(keycode) in "sS":
            self.program.toggleSpecular(0)
            self.update()
        if chr(keycode) in "dD":
            self.program.toggleDiffuse(0)
            self.update()
        if chr(keycode) in "aA":
            self.program.toggleAmbient(0)
            self.update()
