...
class GLProgram:
    ...
    def genFragShaderSource(self):
        ...
        vec3 calcDiffuse(Light light, vec3 normal, vec3 lightDir) {{
            float diff = max(dot(normal, lightDir), 0.0);
            return light.color.rgb * diff;
        }}

        vec3 calcSpecular(Light light, vec3 viewDir, vec3 normal, vec3 lightDir, float highlight) {{
            vec3 reflectDir = reflect(-lightDir, normal);
            float spec = pow(max(dot(viewDir, reflectDir), 0.0), highlight);
            return light.color.rgb * spec;
        }}
        ...
        vec3 calcLighting(vec3 normal, vec3 viewDir, Material material) {{
            vec3 result = vec3(0.0);
            
            // Iterate through all lights in the Light array
            for (int i = 0; i < MAX_LIGHT_NUM; i++) {{
                // Check if the light is enabled
                if (light[i].color.a > 0.0) {{
                    vec3 lightDir;
                    
                    // Check light type and calculate direction accordingly
                    ...
                    // Diffuse component
                    result += calcDiffuse(light[i], normal, lightDir);

                    // Specular component
                    result += calcSpecular(light[i], viewDir, normal, lightDir, material.highlight);
                    
                    // Ambient component
                    result += calcAmbient(light[i]);
                }}
            }}
        ...
        {{
            ...
            //   2. Use the input normal map (â€œ./assets/normalmap.jpgâ€) on both the sphere and the torus.

            
            // Reserved for illumination rendering, routing name is "lighting" or "illumination"
            if ((renderingFlag >> 0 & 0x1) == 1){{
                vec3 lightingResult = calcLighting(normalize(vNormal), normalize(viewPosition - vPos), material);
                ...
                //   Specular, and Ambient. Youâ€™ll implement the missing part in the Fragment shader source code. 
            ...
            if ((renderingFlag >> 3 & 0x1) == 1){{
                ...
                // Requirements:
                //   As a visual debugging mode, youâ€™ll implement a rendering mode that visualizes the vertex normals 
                ...
                // offset and rescale the normal to the range [0, 1]
                vec3 normalizedNormal = normalize(vNormal);
                vec3 color = 0.5 * normalizedNormal + 0.5;
    ...
    #added helper method to toggle lights
    def toggleDiffuse(self, lightIndex):
        light = self.getLight(lightIndex)
        light.toggleDiffuse()
        self.setLight(lightIndex, light)
        self.setInt("toggleDiffuse", light.diffuseOn, lookThroughAttribs=False)

    def toggleAmbient(self, lightIndex):
        light = self.getLight(lightIndex)
        light.toggleAmbient()
        self.setLight(lightIndex, light)
        self.setInt("toggleAmbient", light.ambientOn, lookThroughAttribs=False)

    def toggleSpecular(self, lightIndex):
        light = self.getLight(lightIndex)
        light.toggleSpecular()
        self.setLight(lightIndex, light)
        self.setInt("toggleSpecular", light.specularOn, lookThroughAttribs=False)


    def getLight(self, lightIndex):
        light = Light()
        light.position = self.getVec3(f"{self.attribs['light']}[{lightIndex}].position", lookThroughAttribs=False)
        light.color = self.getVec4(f"{self.attribs['light']}[{lightIndex}].color", lookThroughAttribs=False)
        light.infiniteOn = self.getBool(f"{self.attribs['light']}[{lightIndex}].infiniteOn", lookThroughAttribs=False)
        light.infiniteDirection = self.getVec3(f"{self.attribs['light']}[{lightIndex}].infiniteDirection", lookThroughAttribs=False)
        light.spotOn = self.getBool(f"{self.attribs['light']}[{lightIndex}].spotOn", lookThroughAttribs=False)
        light.spotDirection = self.getVec3(f"{self.attribs['light']}[{lightIndex}].spotDirection", lookThroughAttribs=False)
        light.spotRadialFactor = self.getVec3(f"{self.attribs['light']}[{lightIndex}].spotRadialFactor", lookThroughAttribs=False)
        light.spotAngleLimit = self.getFloat(f"{self.attribs['light']}[{lightIndex}].spotAngleLimit", lookThroughAttribs=False)
        light.diffuseOn = self.getBool(f"{self.attribs['light']}[{lightIndex}].diffuseOn", lookThroughAttribs=False)
        light.specularOn = self.getBool(f"{self.attribs['light']}[{lightIndex}].specularOn", lookThroughAttribs=False)
        light.ambientOn = self.getBool(f"{self.attribs['light']}[{lightIndex}].ambientOn", lookThroughAttribs=False)
        return light
    ...
    def setLight(self, lightIndex: int, light: Light):
        ...
        self.setVec3(f"""{self.attribs["light"]}[{lightIndex}].spotRadialFactor""", light.spotRadialFactor, False) # this line was changed
